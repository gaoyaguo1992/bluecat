// 获取微信配置
 ...
 var shopKeeperType="0";
  $(function(){
	  ...
	  getDeviceData();
	  // 点击跳转设置
	  $('.promt-list1').on('click','li',function(){
		  var i=$(this).index();
		  if(i===0){
			  window.location.href=path+"/wx/merchantCoporateAgentsSetCost.jsp?deviceId="+selectDeviceId+'&custNo='+custNo+'&merchantId='+merchantId+'&openId='+openId+'&deviceType='+selectDeviceType;
		  }else if(i===1){
			  window.location.href=path+"/wx/merchantCoporateAgentsSetProfit.jsp?deviceId="+selectDeviceId+'&custNo='+custNo+'&merchantId='+merchantId+'&openId='+openId;
		  }else if(i===2){
			  window.location.href=path+"/wx/merchantCoporateAgentsBindingMearchant.jsp?deviceId="+selectDeviceId+'&custNo='+custNo+'&merchantId='+merchantId+'&openId='+openId;
		  }else if(i===3){
			  window.location.href=path+"/wx/merchantCoporateAgentsUnBindingMearchant.jsp?deviceId="+selectDeviceId+'&custNo='+custNo+'&merchantId='+merchantId+'&openId='+openId;
		  }else if(i===4){
			  window.location.href=path+"/wx/merchantCoporateAgentsSetGroundingOrNot.jsp?deviceId="+selectDeviceId+'&custNo='+custNo+'&merchantId='+merchantId+'&openId='+openId;
		  }
	  });
	  // 搜索
	 ...
	 $.JsonSRpc(reqUrl+"/wx/myDeviceData",dataObj,function(data){
    	...
    	$.each(deviceList,function(i,list){
    		if(list.status==11){// 未上架
    			$('.dev-list').append(
        				'<div class="devinfor">'+
        			      //'<h2><span style="color:#F2ACAC;border: 1px solid #F2ACAC; " class="isOn">'+list.statusCn+'</span>设备编号'+list.id+'<span onclick="showMode('+list.id+','+list.devType+')" class="set">设置></span></h2>'+
    			...
    			$('.dev-list').append(
    				'<div class="devinfor">'+
      			        //'<h2><span style="color:#22ac38;border: 1px solid #22ac38;" class="isOn">'+list.statusCn+'</span>设备编号'+list.id+'<span onclick="showMode('+list.id+','+list.devType+')" class="set">设置></span></h2>'+
...
// 使mode显示
function showMode(id,deviceType){
	selectDeviceId=id;
