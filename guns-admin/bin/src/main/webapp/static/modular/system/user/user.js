 ...
 * MgrUser设置用户权限。。。
 */
MgrUser.setDataAuthority=function(){
	 if (this.check()) {
	        var index = layer.open({
	            type: 2,
	            title: '配置数据权限（绑定有权限查看的商户）',
		        area: ['800px', '520px'], // 宽高
	            fix: false, //不固定
	            maxmin: true,
	            content: Feng.ctxPath + '/mgr/use_assign_data/' + this.seItem.id
	        });
...
MgrUser.assign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/mgr/use_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.initColumn = function () {
    var columns = [
        ...
        {title: '商户类型', field: 'merchantTypeCn', align: 'center', valign: 'middle', sortable: false},
